function [cbm, success] = cbm_lap(data, model, prior, fname, pconfig, fitOpts)
...
%----------------------------------
% test the model
ok = cbm_check_input(mu',model,data,fname,fitOpts);
if ~ok
else
...
for n=1:N
    ...
    dat = data{n};    
    [loglik_n,theta_n,A_n,G_n,flag_n] = cbm_quad_lap(dat,model,prior,pconfig,0,'LAP',fitOpts);
    ...
    if flag_n~=1 && ~isempty(pconfig.tolgrad_liberal)
        ...
        for tg=1:length(pconfig.tolgrad_liberal)
            if flag_n~=1
                ...
                [loglik_n,theta_n,A_n,G_n,flag_n] = cbm_quad_lap(dat,model,prior,ptconfig,0,'LAP',fitOpts);
                theta_n_l{l} = theta_n;
    ...
    if(~flag_n)
        ...
        if pconfig.prior_for_bads
            ...
            theta_n   = mu;
            [loglik_n]  = cbm_loggaussian(theta_n',model,prior,dat,fitOpts);
            A_n       = A;
            G_n       = nan(1,d);
        else
