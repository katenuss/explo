function [F,G,H] = cbm_loggaussian(theta,model,prior,data,Gconf,Hconf,fitOpts)
% computes log_gaussian of a joint model and prior
% 
% implemented by Payam Piray, Aug 2018
%==========================================================================
if (nargin<5 || isempty(Gconf)), Gconf = 'off'; end
if (nargin<6 || isempty(Hconf)), Hconf = 'off'; end
...
switch mode
    case 1
        [F] = model(theta,data,fitOpts); % returing logpdf(X|theta)
        G   = nan;
        H   = nan;
    case 2
        [F,G]   = model(theta,data,fitOpts); % returing logpdf(X|theta)
        H       = nan;
    case 3
        [F,G,H] = model(theta,data,fitOpts); % returing logpdf(X|theta)
end
