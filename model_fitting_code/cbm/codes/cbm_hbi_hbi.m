...
while ~terminate && iter<=maxiter
    ...
    [qhquad] = hbi_qhquad(models,data,config.optconfigs,qmutau,qhquad,fid,opt_parallel,fitOpts);
...
%==========================================================================
function [qh_new] = hbi_qhquad(models,data,pconfig,qmutau,qh,fid,opt_paral,fitOpts)
...
if opt_paral
    try
    [logf,theta,A,~,flag] = cbm_loop_quad_lap(data,models,priors,allconfigs,fid,'LAP',fitOpts);
    catch
...
%     A     = repmat({nan(D(k),D(k))},K,N);    
    ...
    for k=1:K
        for n=1:N
            [logf(k,n),theta{k}(:,n),A{k,n},~,flag(k,n)] = cbm_quad_lap(data{n},models{k},priors(k),pconfig(k),fid,'LAP',fitOpts{k}); 
        end
    end
end
...
for n=1:N
    for k=1:K
        if flag(k,n)<1 % if no good gradient found (i.e. optimization failed), use the prior values for this subject-model
            ...
            [logf(k,n)]  = cbm_loggaussian(theta_n',models{k},priors(k),data{n},[], [], fitOpts{k});    
